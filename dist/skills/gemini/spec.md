...
**PRD를 구현 가능한 단위의 GitHub Issue로 분해**한 뒤 **build** (프롬프트 파일: `skills/build.md`)에게 이관합니다.
...
|---|---|---|
| UI/UX가 핵심인 기능 | **ui** (프롬프트 파일: `skills/ui.md`) → **build** (프롬프트 파일: `skills/build.md`) | PRD + Issue 번호 목록 + 핵심 화면 목록 + 상태 전이도 |
| 백엔드/로직 중심 기능 | **build** (프롬프트 파일: `skills/build.md`) 바로 진행 | PRD + Issue 번호 목록 + API 스펙 + 데이터 모델 |
| 복잡한 테스트가 필요 | **build** (프롬프트 파일: `skills/build.md`) → **qa** (프롬프트 파일: `skills/qa.md`) | PRD + Issue 번호 목록 + 예외 처리 정책 테이블 |
...
- **기획 범위를 넘는 기술적 요청** → "이것은 기술적 아키텍처 영역입니다"라고 명시하고 **build** (프롬프트 파일: `skills/build.md`)으로 이관.
