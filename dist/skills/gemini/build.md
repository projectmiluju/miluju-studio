...
**spec** (프롬프트 파일: `skills/spec.md`)가 정의한 요구사항과 **ui** (프롬프트 파일: `skills/ui.md`)가 만든 디자인 시스템을 받아,
...
**입력:** **spec** (프롬프트 파일: `skills/spec.md`)의 PRD + GitHub Issue + **ui** (프롬프트 파일: `skills/ui.md`)의 디자인 시스템/컴포넌트 명세서

| 항목 | 부족하면 |
|------|---------|
| GitHub Issue (이슈 번호 + 완료 조건) | **spec** (프롬프트 파일: `skills/spec.md`)에게 이슈 생성 요청 |
| PRD (요구사항 명확) | **spec** (프롬프트 파일: `skills/spec.md`)에게 돌려보내기 |
| 예외 처리 정책 | 직접 역질문 |
| 디자인 토큰 | **ui** (프롬프트 파일: `skills/ui.md`)에게 돌려보내기 |
| 컴포넌트 명세서 | **ui** (프롬프트 파일: `skills/ui.md`)에게 돌려보내기 |
| 기술 스택 | 직접 역질문 후 선정 |
...
| **이슈 전용 브랜치 생성·체크아웃** | 동기화한 기본 브랜치에서 **이슈에 해당하는 새 브랜치를 만들고** `git switch`로 **체크아웃**한 뒤, **그 브랜치에서만** 구현을 시작합니다. 브랜치 이름은 **ship** (프롬프트 파일: `skills/ship.md`) 스킬의 네이밍 규칙을 따릅니다(예: `feat/#12-설명`, `fix/#18-설명`). |
...
|----------|---------|---------------|
| 테스트 작성 | **qa** (프롬프트 파일: `skills/qa.md`) | Issue 번호 + 핵심 로직 목록 + PRD 예외 처리 정책 + 엣지 케이스 |
| 기록 | **docs** (프롬프트 파일: `skills/docs.md`) | Issue 번호 + 기술적 결정과 이유 |
| 커밋/PR | **ship** (프롬프트 파일: `skills/ship.md`) | Issue 번호 + 변경 파일 목록 + 커밋 메시지 초안 |
...
| 아키텍처 수준 결정 | STOP. **spec** (프롬프트 파일: `skills/spec.md`)에게 돌려보내기. |
