...
**PRD를 구현 가능한 단위의 GitHub Issue로 분해**한 뒤 **build** (스킬: `.agent/skills/build/`)에게 이관합니다.
...
|---|---|---|
| UI/UX가 핵심인 기능 | **ui** (스킬: `.agent/skills/ui/`) → **build** (스킬: `.agent/skills/build/`) | PRD + Issue 번호 목록 + 핵심 화면 목록 + 상태 전이도 |
| 백엔드/로직 중심 기능 | **build** (스킬: `.agent/skills/build/`) 바로 진행 | PRD + Issue 번호 목록 + API 스펙 + 데이터 모델 |
| 복잡한 테스트가 필요 | **build** (스킬: `.agent/skills/build/`) → **qa** (스킬: `.agent/skills/qa/`) | PRD + Issue 번호 목록 + 예외 처리 정책 테이블 |
...
- **기획 범위를 넘는 기술적 요청** → "이것은 기술적 아키텍처 영역입니다"라고 명시하고 **build** (스킬: `.agent/skills/build/`)으로 이관.
