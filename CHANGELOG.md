...
- **spec 스킬**: GitHub 이슈 생성 시 저장소 `.github/ISSUE_TEMPLATE` 등 **프로젝트 템플릿 우선** 사용. 이슈 템플릿 **신규·수정·삭제는 사용자 승인 후**에만 진행.
- **ship 스킬**: PR 생성 시 `.github/pull_request_template.md` 등 **저장소 PR 템플릿 우선**. PR 템플릿 **신규·수정·삭제는 사용자 승인 후**에만 진행.
- **build 스킬**: 이슈 기반 구현 시 **`main`/`develop`에서 직접 작업 금지**. 기본 브랜치 `fetch`·`pull` 후 **이슈 전용 브랜치** 생성·체크아웃하여 작업.

### Changed

- **README**: 스킬 요약·프로젝트 구조·원격 설치 동작 요약 반영.
- **문서**: 위 스킬 변경에 맞춰 사용자 안내 정리.
